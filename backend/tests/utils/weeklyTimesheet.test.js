...
describe('buildWeeklyTimesheet', () => {
  ...
  it('aplica o buffer de HE ao total do DIA', () => {
    const short = entry({ workedMinutes: 486, overtimeMinutes: 6 });
    const result = buildWeeklyTimesheet({ ...base, bufferMinutes: 10, entries: [short] });
  ...
  it('tira a HE do TOTAL do dia, nao da fatia de cada marcacao', () => {
    ...
    const result = buildWeeklyTimesheet({ ...base, bufferMinutes: 10, entries: [manha, tarde] });

    expect(result.days[0].workedMinutes).toBe(492);
    expect(result.days[0].overtimeMinutes).toBe(12);
  });

  it('conta a HE em cheio acima do buffer (gatilho, nao desconto)', () => {
    const acima = entry({ workedMinutes: 491 });
    const result = buildWeeklyTimesheet({ ...base, bufferMinutes: 10, entries: [acima] });
  ...
  it('soma a HE dia a dia, sem aplicar o buffer sobre a semana', () => {
    // Dois dias com 6min de sobra e buffer de 10: cada dia zera. Se o buffer
