const { applyOvertimeBuffer, resolveBufferMinutes, resolveContractDailyMinutes } = require('./overtime');
const {
...
const buildWeeklyTimesheet = ({
  ...
  const days = Array.from({ length: DAYS_IN_WEEK }, (_, index) => {
    ...
    // A HE sai do TOTAL do dia e o buffer se aplica UMA vez, nunca por
    // marcacao: o buffer e um conceito diario. Somando HE marcacao por
    // marcacao, duas fatias de 6min com buffer de 10 virariam zero cada uma e o
    // dia perderia 12 minutos de hora extra real.
    //
    // Recalcular em vez de somar entry.overtimeMinutes tambem e proposital: o
    // gravado pode ter nascido antes do buffer existir, e o dia aberto ainda
    // nao tem HE gravada nenhuma.
    const overtimeMinutes = applyOvertimeBuffer(
      Math.max(0, workedMinutes - contract),
  ...
  return {
    ...
    // Soma da HE JA com o buffer aplicado, dia a dia. Aplicar o buffer sobre a soma da
    // semana seria outra regra (e ilegal): a tolerancia do art. 58 §1º e diaria.
