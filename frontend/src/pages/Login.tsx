...
const Login = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : tr('Failed to sign in', 'Falha ao entrar'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen px-6 py-10">
      ...
      <div className="mx-auto flex w-full max-w-5xl items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            <p className="text-xs uppercase tracking-[0.3em] text-slate-500">{tr('Time tracking', 'Sistema de ponto')}</p>
            <h1 className="text-xl font-semibold text-slate-900">SystemaPonto</h1>
          </div>
        </div>
        <span className="text-xs uppercase tracking-[0.3em] text-slate-400">{tr('Login', 'Login')}</span>
      </div>
      ...
      <div className="mx-auto mt-14 grid w-full max-w-5xl gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-3xl border border-white/80 bg-white/80 p-8 shadow-[0_16px_40px_-30px_rgba(15,23,42,0.55)] backdrop-blur">
          <h2 className="text-3xl font-semibold text-slate-900">{tr('Welcome back.', 'Bem-vindo de volta.')}</h2>
          <p className="mt-3 text-sm text-slate-600">
            {tr(
              'Sign in to record your workday, review requests, and track reports.',
          ...
          <div className="mt-10 rounded-2xl border border-slate-100 bg-slate-50/70 p-5 text-xs text-slate-500">
            {tr(
              'Tip: use the same email registered in Supabase to authenticate.',
        ...
        <form onSubmit={handleSubmit} className="rounded-3xl border border-slate-100 bg-white/90 p-8 shadow-sm">
          <label className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">{tr('Email', 'Email')}</label>
          <input
            value={email}
            onChange={(event) => setEmail(event.target.value)}
            type="email"
            required
            placeholder={tr('email@company.com', 'email@empresa.com')}
          ...
          <label className="mt-6 block text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">{tr('Password', 'Senha')}</label>
          <input
            value={password}
          ...
          >
            {loading ? tr('Signing in...', 'Entrando...') : tr('Sign in', 'Entrar')}
          </button>
        </form>
      </div>
    </div>
