  ...
  return (
    <section className="grid gap-6">
      <div className="rounded-3xl border border-white/80 bg-white/80 p-8 shadow-[0_16px_40px_-30px_rgba(15,23,42,0.55)] backdrop-blur">
        ...
        <h2 className="mt-4 text-3xl font-semibold text-slate-900">Histórico de ponto e ganhos</h2>
        <p className="mt-3 text-sm text-slate-600">Veja quanto você trabalhou e quanto ganhou por dia, com base na sua hora cadastrada.</p>
      </div>

      <div className="rounded-3xl border border-slate-100 bg-white/90 p-6 shadow-sm">
        <div className="flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.2em] text-slate-500">
          <span className="rounded-full bg-slate-100 px-3 py-1">Valor/hora: {hourlyRate > 0 ? formatCurrency(hourlyRate) : 'Nao definido'}</span>
          <span className="rounded-full bg-slate-100 px-3 py-1">Total trabalhado: {formatMinutesLabel(totals.workedMinutes)}</span>
          <span className="rounded-full bg-slate-100 px-3 py-1">Total ganho: {formatCurrency(totals.gainAmount)}</span>
        </div>
        ...
        {loading ? <p className="mt-3 text-sm text-slate-500">Carregando histórico...</p> : null}
        {error ? <p className="mt-3 text-xs text-rose-600">{error}</p> : null}
      </div>

      <div className="rounded-3xl border border-slate-100 bg-white/90 p-6 shadow-sm">
        <h3 className="text-lg font-semibold text-slate-900">Por dia</h3>

        <div className="mt-4 space-y-4">
          {groupedByDay.length === 0 ? (
            <p className="text-sm text-slate-500">Sem registros no período.</p>
          ) : (
            groupedByDay.map((day) => (
              <div key={day.date} className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-sm font-semibold text-slate-900">
                    {day.date === 'unknown' ? 'Data inválida' : formatDateWithTimeZone(`${day.date}T00:00:00.000Z`, viewTimeZone)}
                  </p>
                  <p className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-800">
                    Ganho: {formatCurrency(day.gainAmount)}
                  </p>
                </div>

                <div className="mt-2 grid gap-2 text-xs text-slate-600 sm:grid-cols-3">
                  <span className="rounded-full bg-white px-3 py-1">Trabalhado: {formatMinutesLabel(day.workedMinutes)}</span>
                  <span className="rounded-full bg-white px-3 py-1">HE 50%: {formatMinutesLabel(day.overtimeMinutes50)}</span>
                  <span className="rounded-full bg-white px-3 py-1">HE 100%: {formatMinutesLabel(day.overtimeMinutes100)}</span>
                </div>
                  ...
                  {day.entries.map((entry) => (
                    <div key={entry.id} className="rounded-xl border border-slate-100 bg-white px-3 py-2 text-xs text-slate-600">
                      ...
                      <p className="mt-1 text-[11px] text-slate-500">
                        Ganho no registro: {formatCurrency(calculateGain(entry, hourlyRate))}
                      </p>
                    </div>
                  ))}
                </div>
