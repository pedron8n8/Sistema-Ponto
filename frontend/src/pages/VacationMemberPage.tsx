...
const VacationMemberPage = () => {
  ...
  const handleCreate = async () => {
    ...
    if (!form.startDate || !form.endDate) {
      setError('Informe data inicial e final para solicitar férias.')
      return
    }
    ...
    try {
      ...
      setNotice('Solicitação de férias enviada com sucesso.')
  ...
  return (
    <section className="grid gap-6">
      <div className="rounded-3xl border border-white/80 bg-white/80 p-8 shadow-[0_16px_40px_-30px_rgba(15,23,42,0.55)] backdrop-blur">
        <p className="text-xs uppercase tracking-[0.35em] text-teal-700">Férias</p>
        <h2 className="mt-4 text-3xl font-semibold text-slate-900">Solicite e acompanhe suas férias.</h2>
        <p className="mt-3 text-sm text-slate-600">
          O fluxo segue para aprovação do supervisor e confirmação final do RH.
        </p>
      </div>

      <div className="rounded-3xl border border-slate-100 bg-white/90 p-6 shadow-sm">
        <h3 className="text-lg font-semibold text-slate-900">Nova solicitação</h3>
      ...
      <div className="rounded-3xl border border-slate-100 bg-white/90 p-6 shadow-sm">
        ...
        <div className="mt-4 space-y-2 text-xs text-slate-600">
          {loading ? <p className="text-sm text-slate-500">Carregando solicitações...</p> : null}
          {!loading && requests.length === 0 ? <p>Nenhuma solicitação registrada.</p> : null}
